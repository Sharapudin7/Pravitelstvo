...
<section class="list">
	<? foreach ($arResult["ITEMS"] as $arItem) : ?>
		<? $arElement = $arItem['DISPLAY_PROPERTIES'] ?>
		<div class="list__item list__item--documents">
			<div class="list__detail">
				...
				<div class="list__foot">
					<div class="meta">
						<div class="list__date meta__item" datetime="2021-11-16">
							<span>Дата: <?= $arElement['DOCUMENT_DATE']['VALUE'] ?></span>
						</div>
						<? if (!empty($arElement['DOCUMENT_NUMBER'])) { ?>
							<div class="meta__item">
								<span>№:</span>
								<span><?= $arElement['DOCUMENT_NUMBER']['VALUE'] ?></span>
							</div>
						<? } ?>
						<? if (!empty($arElement['DOCUMENT_FILE'])) { ?>
							<div class="meta__item">
								<span>Тип:</span>
								<a href="<?= $arElement['DOCUMENT_FILE']['FILE_VALUE']['SRC'] ?>"><?= $arElement['DOCUMENT_SECTION']['VALUE'] ?></a>
							</div>
						<? } ?>
						<div class="meta__item">
							...
							<a href="<?= $arElement['DOCUMENT_FILE']['FILE_VALUE']['SRC'] ?>" target="_blank" download="">Скачать</a>
						</div>
					</div>
				</div>
			</div>
